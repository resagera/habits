...
//	              вид: video | clips | music | photo (по умолчанию video);
//	              clips — видеоролики: без поиска постеров в интернете
...
//	MEDIA_ADMIN_PIN  PIN админки на странице (переименование, удаление);
//	              не задан — админка закрыта
//	MEDIA_TRASH_DAYS через сколько дней удалённое стирается с диска, по умолчанию 3
package main
...
type config struct {
	...
	// имя из кэша.
	roomOverride string
	// adminPin — PIN админки. Пусто — админка выключена: в домашнем вайфае
	// живёт не только приставка, а там можно удалять сериалы.
	adminPin string
	// trashDays — сколько дней удалённое лежит в корзине до стирания.
	trashDays int
}
...
func loadConfig() config {
	...
	c := config{
		...
		adminPin:     pick(file, "MEDIA_ADMIN_PIN", "admin_pin", ""),
		trashDays:    3,
	}
	if n, err := strconv.Atoi(pick(file, "MEDIA_TRASH_DAYS", "trash_days", "3")); err == nil && n >= 1 && n <= 60 {
		c.trashDays = n
	}
...
func parseRoots(spec string) []library {
	...
	for _, part := range strings.Split(spec, ";") {
		...
		if len(fields) > 2 {
			switch k := strings.TrimSpace(fields[2]); k {
			case "music", "photo", "video", "clips":
				kind = k
			case "clip":
				kind = "clips"
			}
		...
		// Недоступную папку НЕ выбрасываем: внешний диск монтируется
		// udisks2 уже после входа в сессию, а user-сервис стартует раньше —
		// выброшенная на старте библиотека не появлялась до перезапуска.
		// Наличие проверяется при каждом запросе списка (library.available).
		if title == "" {
...
func main() {
	...
	for _, r := range cfg.roots {
		state := ""
		if !r.available() {
			state = "  (пока недоступна — подхватится, когда появится)"
		}
		log.Printf("  %s [%s] %s%s", r.Title, r.Kind, r.Path, state)
	}
	...
	if cfg.adminPin == "" {
		log.Printf("админка выключена: задайте admin_pin в настройках")
	} else {
		log.Printf("админка включена, удалённое стирается через %d дн.", cfg.trashDays)
	}
