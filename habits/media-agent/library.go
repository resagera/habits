...
// available — примонтирована ли папка прямо сейчас. Библиотеки на съёмных
// дисках появляются позже старта агента и могут исчезать: список проверяется
// на каждый запрос, а не один раз при запуске.
