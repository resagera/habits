...
func newServer(cfg config) *server {
	...
	s := &server{cfg: cfg, mux: http.NewServeMux(), q: newQueue(cfg.cache),
		secret: loadSecret(cfg.cache), progress: newProgressStore(cfg.cache),
		trash: newTrashStore(cfg.cache), looks: newLooksStore(cfg.cache), marks: newMarksStore(cfg.cache),
		playlists: newPlaylistStore(cfg.cache), statsStore: newStatsStore(cfg.cache),
		scan: &scanJob{}}
	...
	s.mux.HandleFunc("GET /api/item", s.item)
	s.mux.HandleFunc("GET /api/next-season", s.nextSeason)
	s.mux.HandleFunc("GET /poster/{id}", s.poster)
	s.mux.HandleFunc("GET /api/photos", s.photos)
	s.mux.HandleFunc("GET /photo/{id}", s.photo)
	s.mux.HandleFunc("GET /api/progress", s.listProgress)
	s.mux.HandleFunc("PUT /api/progress", s.saveProgress)
	s.mux.HandleFunc("POST /api/progress", s.saveProgress)
	s.mux.HandleFunc("DELETE /api/progress/{key}", s.deleteProgress)
	s.mux.HandleFunc("GET /app/habits-tv.apk", s.appAPK)
	s.mux.HandleFunc("HEAD /app/habits-tv.apk", s.appAPK)
	...
	s.mux.HandleFunc("GET /background", s.background)
	s.mux.HandleFunc("GET /api/marks", s.getMarks)
	s.mux.HandleFunc("GET /api/marks/chapters", s.chapterMarks)
	s.mux.HandleFunc("GET /api/info", s.info)
	s.mux.HandleFunc("GET /api/stats", s.stats)
	s.playlistRoutes()
	s.uploadRoutes()
	s.mux.HandleFunc("PUT /api/marks", s.putMarks)
	s.adminRoutes()
	return s
}
...
func (s *server) libraries(w http.ResponseWriter, r *http.Request) {
	// показываем только те, что сейчас на месте: диск могли не примонтировать
	// к старту агента или отключить на ходу
	out := make([]library, 0, len(s.cfg.roots))
...
// appAPK — приложение для приставки (habits/tv-app, выкладывается build.sh
// --deploy в кэш агента). Страница проверяет его HEAD-запросом и показывает
// значок Android в шапке, только если файл есть.
func (s *server) appAPK(w http.ResponseWriter, r *http.Request) {
	f, err := os.Open(filepath.Join(s.cfg.cache, "app", "habits-tv.apk"))
	if err != nil {
		http.NotFound(w, r)
		return
	}
	...
	w.Header().Set("Content-Disposition", `attachment; filename="habits-tv.apk"`)
	// новая сборка ложится под тем же именем — кэшировать нельзя
	w.Header().Set("Cache-Control", "no-cache")
	http.ServeContent(w, r, "habits-tv.apk", st.ModTime(), f)
}
...
func contentType(path string) string {
	...
	case ".m4v", ".mov":
		// ролики с iPhone — тот же MP4 внутри; «video/quicktime» Chrome не берётся играть
		return "video/mp4"
	case ".ts":
		return "video/mp2t"
	case ".opus":
