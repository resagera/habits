...
# Почта: приёмник должен отвечать баннером на 25 порту. Проверяем снаружи —
# именно так к нему постучится чужой почтовый сервер.
smtp_banner=$($SSH "exec 3<>/dev/tcp/127.0.0.1/25 && head -1 <&3" 2>/dev/null || true)
case "$smtp_banner" in
  220*) echo "  почта (SMTP 25): $smtp_banner" ;;
  *)    echo "  !! SMTP 25 не отвечает баннером 220 (получено: '${smtp_banner:-пусто}')"; exit 1 ;;
esac
