...
import {
  applyScale, applyTokens, builtinTheme, defaultTokens,
...
export interface AppearanceState {
  ...
  bg_off?: boolean
  /** масштаб интерфейса в процентах (пусто или 100 — как есть) */
  scale?: number
}
...
export function apply(): void {
  ...
  applyScale(state.value.scale ?? 0)
}
...
/**
 * Заменить состояние оформления целиком — перенос настроек и сброс. Обычные
 * правки идут через selectTheme/editToken: там состояние меняется по кусочку,
 * а здесь приходит готовое.
 */
export function setAppearanceState(next: AppearanceState): Promise<void> {
  return push(next)
}

/** Масштаб интерфейса в процентах (100 — как есть). */
export const scale = computed(() => state.value.scale || 100)

export function setScale(percent: number): Promise<void> {
  return push({ ...state.value, scale: percent })
}
