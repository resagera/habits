...
// activeTheme нужен автосмене: у тёмной и светлой темы могут быть разные папки.
// Импорт статический, хотя модули ссылаются друг на друга: обращение идёт только
// из функций, а не во время вычисления модуля.
import { activeTheme, bgHidden } from './appearance'
...
export interface BackgroundImageItem {
  ...
  thumb?: string
  /** вес файла в байтах (сервер считает его со диска) */
  size?: number
}

/**
 * Автосмена фона. Папка хранится числом: -1 — все картинки, 0 — «без папки»,
 * иначе id папки. Отдельные источники для тёмной и светлой темы дают и
 * «каждый день новая картинка», и «свои фоны под каждую тему».
 */
export interface BackgroundAuto {
  mode: 'off' | 'open' | 'daily'
  folder_dark: number
  folder_light: number
  /** YYYY-MM-DD последней смены ('' — ещё не менялся) */
  day: string
}

export const DEFAULT_AUTO: BackgroundAuto = {
  mode: 'off', folder_dark: -1, folder_light: -1, day: '',
}
...
/** Настройки автосмены последнего ответа сервера — их читает и страница, и наблюдатель за темой. */
export const bgAuto = ref<BackgroundAuto>({ ...DEFAULT_AUTO })
...
function cacheAndApply(state: BackgroundState): void {
  if (state.auto) bgAuto.value = state.auto
...
export async function setBackgroundAuto(auto: BackgroundAuto): Promise<BackgroundState> {
  const state = await api.put<BackgroundState>('/settings/background/auto', auto)
  cacheAndApply(state)
  return state
}

/**
 * Сменить картинку по правилу автосмены. Решает сервер: «раз в день» он
 * сверяет со своей датой, иначе два устройства в один день выбрали бы разные
 * картинки и перетёрли друг друга.
 */
export async function rotateBackground(mode: 'open' | 'daily' | 'force'): Promise<BackgroundState> {
  const scheme = activeTheme.value.kind === 'light' ? 'light' : 'dark'
  const state = await api.post<BackgroundState>(
    `/settings/background/rotate?scheme=${scheme}&mode=${mode}`, {},
  )
  cacheAndApply(state)
  return state
}
...
/**
 * Запуск автосмены: один раз при открытии приложения и дальше при каждой
 * смене светлой/тёмной темы, если для них выбраны разные папки.
 */
export async function startBackgroundAuto(): Promise<void> {
  if (bgAuto.value.mode === 'off') return
  try {
    await rotateBackground(bgAuto.value.mode)
  } catch {
    /* нет сети — фон просто остаётся прежним */
  }
  watch(() => activeTheme.value.kind, (next, prev) => {
    if (next === prev) return
    const a = bgAuto.value
    if (a.mode === 'off' || a.folder_dark === a.folder_light) return
    rotateBackground('force').catch(() => {})
  })
}
