...
/**
 * Масштаб интерфейса. Меняем zoom, а не размер шрифта: размеры в приложении
 * заданы в пикселях, и font-size на корне подвинул бы только те тексты, где
 * он не указан явно. zoom пересчитывает разом и шрифты, и отступы.
 */
export function applyScale(percent: number): void {
  const root = document.documentElement
  if (!percent || percent === 100) root.style.removeProperty('zoom')
  else root.style.zoom = String(percent / 100)
}

/** Умеет ли браузер zoom (Safari до 18 и старые Firefox — нет). */
...
/**
 * Подобрать цвет текста, читаемый на этом фоне: оттенок и насыщенность
 * сохраняем, двигаем только светлоту — в сторону от фона, пока контраст не
 * дотянет до порога. Если не дотягивает вовсе (серый текст на сером фоне),
 * возвращаем чистый белый или чёрный.
 */
export function fixContrast(text: string, bg: string, target = 4.5): string {
  const bgRgb = parseColor(bg)
  const hslText = hexToHsl(text)
  if (!bgRgb || !hslText) return text
  const dark = luminance(bgRgb) < 0.5 // тёмный фон — текст светлее, и наоборот
  const [h, sat] = hslText
  for (let step = 0; step <= 20; step++) {
    const l = dark ? Math.min(1, hslText[2] + step * 0.05) : Math.max(0, hslText[2] - step * 0.05)
    const candidate = hsl(h, sat, l)
    const r = contrastRatio(candidate, bg)
    if (r !== null && r >= target) return candidate
  }
