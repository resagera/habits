...
import { parseAny, toText } from '../transfer'
import type { ExportSubgroup } from '../transfer'
import type { CheckGroup } from '../types'

// всего пунктов в шаблоне (с учётом подгрупп)
function templateCount(t: { items: unknown[]; subgroups: ExportSubgroup[] }): number {
  let n = t.items.length
  for (const s of t.subgroups ?? []) n += templateCount(s)
  return n
}
// тело шаблона в текст (- пункт / # подгруппа), без строки-имени
function templateToText(t: CheckTemplate): string {
  return toText({ name: '.', items: t.items, subgroups: t.subgroups }).split('\n').slice(1).join('\n')
}
...
function openEdit(t: CheckTemplate) {
  editing.value = t
  name.value = t.name
  bodyText.value = templateToText(t)
  confirmDelete.value = false
  modal.value = true
}
...
async function save() {
  ...
  // разбираем текст «- пункт / # подгруппа» в дерево (имя добавляем первой строкой)
  const tree = parseAny(n + '\n' + bodyText.value)
  const body = { items: tree.items, subgroups: tree.subgroups }
  try {
    if (editing.value) {
      const { template } = await checkerApi.updateTemplate(editing.value.id, n, body)
      const i = templates.value.findIndex((x) => x.id === template.id)
      if (i >= 0) templates.value[i] = template
    } else {
      const { template } = await checkerApi.createTemplate(n, body)
...
<template>
    ...
    <template v-if="open">
      ...
      <div v-for="t in templates" :key="t.id" class="tpl-row">
        <div class="tpl-info">
          ...
          <div class="tpl-items">
            {{ templateCount(t) }} пунктов<span v-if="t.subgroups.length"> · с подгруппами</span>
          </div>
        </div>
  ...
  <div v-if="modal" class="modal" @click.self="modal = false">
    <div class="modal-content tpl-modal">
      ...
      <label class="field-label">Пункты «- …», подгруппы «# …» (## — глубже)</label>
      <textarea
        v-model="bodyText"
        rows="8"
        placeholder="- Паспорт&#10;- Зарядка&#10;# Документы&#10;- Билеты"
