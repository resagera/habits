...
import { colorValue } from '../display'
...
export function useTracker() {
  ...
  /**
   * Активный цвет трекера. Если задан набор именованных цветов, активным
   * может быть только цвет из набора: иначе после удаления цвета из набора
   * отметки продолжали бы ставиться цветом, которого в наборе уже нет.
   */
  function activeColor(cat: Category): string {
    const saved = active[cat.id]?.color
    if (cat.palette.length > 0) {
      const values = cat.palette.map(colorValue)
      return saved && values.includes(saved) ? saved : values[0]
    }
    return saved ?? cat.color
  }
  ...
  /**
   * «Последние» цвета для пикера — реально использованные в этом трекере,
   * свежие первыми. Раньше список брался только из localStorage, а его в
   * вебвью Telegram периодически чистит, и вместо «последних» человек видел
   * пустоту и общую палитру пресетов. Отметки — источник надёжнее; список с
   * устройства идёт следом, он помнит и то, что вышло за окно загрузки.
   */
  function recentColorsFor(cat: Category): string[] {
    const used = [...(marks.get(cat.id)?.entries() ?? [])]
      .filter(([, info]) => info.color)
      .sort(([a], [b]) => (a < b ? 1 : a > b ? -1 : 0))
      .map(([, info]) => info.color as string)
    const out: string[] = []
    for (const c of [...used, ...recentColors.value]) {
