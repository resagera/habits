...
import {
  ...
  saveThemeAs, scale, selectAuto, selectTheme, setBgOff, setDraft, setScale,
  state, themeById, themeBgUrl,
} from '../../../shared/appearance'
import {
  BUILTIN_THEMES, contrastRatio, fixContrast, generateTheme, scaleSupported,
  TOKEN_LABELS, type Theme, type ThemeKind, type ThemeTokens,
} from '../../../shared/themes'
import ThemeSample from './ThemeSample.vue'
...
/** Подобрать читаемый цвет текста: правится только он, фон остаётся замыслом. */
function repairContrast() {
  return guard(async () => {
    await editToken('text', fixContrast(tokens.value.text, tokens.value.bg))
    showToast('Цвет текста подобран')
  })
}

const canScale = scaleSupported()
const scalePresets = [
  { value: 90, label: 'Компактный' },
  { value: 100, label: 'Обычный' },
  { value: 115, label: 'Крупный' },
]

function onScale(percent: number) {
  return guard(() => setScale(percent))
}
...
<template>
  <div class="ap">
    ...
    <div class="sub">
      ...
      <p class="hint">Как это выглядит</p>
      <ThemeSample />
    </div>
    ...
    <div class="sub">
      ...
      <p v-if="contrast !== null" class="hint" :class="{ warn: contrast < 4.5 }">
        ...
        <button v-if="contrast < 4.5" class="btn small" :disabled="busy" @click="repairContrast">
          Починить контраст
        </button>
      </p>
    </div>
    ...
    <!-- масштаб интерфейса -->
    <div class="sub">
      <h4>Размер интерфейса</h4>
      <p class="hint">
        Меняются разом и шрифт, и отступы — как масштаб страницы в браузере.
        Настройка своя у Telegram и у браузера.
      </p>
      <template v-if="canScale">
        <div class="gen-row">
          <button v-for="p in scalePresets" :key="p.value" class="btn small"
                  :class="{ primary: scale === p.value }" :disabled="busy"
                  @click="onScale(p.value)">
            {{ p.label }}
          </button>
        </div>
        <label class="slider">
          <span>Масштаб: {{ scale }}%</span>
          <input :value="scale" type="range" min="70" max="150" step="5" :disabled="busy"
                 @change="onScale(Number(($event.target as HTMLInputElement).value))" />
        </label>
      </template>
      <p v-else class="hint warn">
        Этот браузер не умеет менять масштаб страницы — настройка работать не будет.
      </p>
    </div>
