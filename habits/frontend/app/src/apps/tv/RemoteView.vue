...
/** Плеер в комнате: у одного агента их бывает несколько — ТВ и компьютер. */
...
/** Показ с телефона на экране приставки. */
interface Cast { video: boolean; paused: boolean; time?: number; duration?: number }
interface Job {
...
// каким экраном управляем: пустая строка — экранов ещё нет
const screen = ref('')
...
/**
 * «Отправить на ТВ»: фото или ролик из галереи — сразу на экран приставки.
 * Файл на пару часов ложится на прод, приставка забирает его оттуда: телефон
 * может быть и не в домашней сети. XHR, а не fetch, — ради прогресса отдачи.
 */
const castInput = ref<HTMLInputElement | null>(null)
const castProgress = ref<number | null>(null)

function pickCast() {
  castInput.value?.click()
}

function sendCast(e: Event) {
  const input = e.target as HTMLInputElement
  const file = input.files?.[0]
  input.value = ''
  if (!file || !lastKey) return
  if (!/^(image|video)\//.test(file.type)) {
    showToast('Нужно фото или видео')
    return
  }
  if (file.size > 500 * 1024 * 1024) {
    showToast('Файл больше 500 МБ')
    return
  }
  castProgress.value = 0
  const xhr = new XMLHttpRequest()
  const to = screen.value ? `&screen=${encodeURIComponent(screen.value)}` : ''
  xhr.open('POST', `${apiBase()}/tv/cast?room=${encodeURIComponent(lastKey)}&name=${encodeURIComponent(file.name)}${to}`)
  xhr.setRequestHeader('Authorization', apiAuthHeader())
  xhr.setRequestHeader('Content-Type', file.type)
  xhr.upload.onprogress = (ev) => {
    if (ev.lengthComputable) castProgress.value = Math.round((ev.loaded / ev.total) * 100)
  }
  xhr.onload = () => {
    castProgress.value = null
    if (xhr.status < 300) {
      showToast('📺 На экране')
      return
    }
    let msg = 'Не отправилось'
    try {
  ...
  xhr.onerror = () => {
    castProgress.value = null
    showToast('Связь оборвалась — попробуйте ещё раз')
  }
  xhr.send(file)
}
...
async function stopCast() {
  if (!lastKey) return
  try {
    await api.post(`/tv/cast/stop?room=${encodeURIComponent(lastKey)}${screen.value ? `&screen=${encodeURIComponent(screen.value)}` : ''}`)
  } catch (e) {
    showToast(e instanceof Error ? e.message : 'Не получилось')
  }
}

/** Код часто пересылают себе в мессенджер с экрана — вставка одним нажатием. */
async function pasteCode() {
  ...
  // прочитать буфер не дали (Telegram так умеет) — вставка долгим нажатием
  // на поле прав не требует, туда и отправляем
  codeInput.value?.focus()
  showToast('Буфер не читается — вставьте долгим нажатием на поле')
}
...
function onMessage(m: Record<string, unknown>) {
  // ответ другого экрана той же комнаты — не наш: иначе название фильма
  // мелькало бы, показывая то один плеер, то другой
  if (m.s && screen.value && m.s !== screen.value) return
  switch (m.t) {
    case 'presence': {
      tvOnline.value = Number(m.tv) > 0
      screens.value = (m.screens as Screen[]) ?? []
      // выбранный экран закрыли — переходим на первый оставшийся
      if (!screens.value.some((x) => x.id === screen.value)) {
        screen.value = screens.value[0]?.id ?? ''
        video.value = null
        music.value = null
      }
      break
    ...
    case 'state':
      ...
      cast.value = (m.cast as Cast) ?? null
      break
    case 'jobs':
...
function send(obj: Record<string, unknown>) {
  // «to» — кому: команда уходит выбранному экрану, а не всем плеерам комнаты
  const msg = screen.value ? { ...obj, to: screen.value } : obj
  if (ws && ws.readyState === 1) ws.send(JSON.stringify(msg))
  else showToast('Нет связи с приставкой')
}

/** Переключиться на другой экран: показания прежнего сразу забываем. */
function pickScreen(id: string) {
  ...
  jobs.value = []
  send({ t: 'hello' })
  browse(stack.value[stack.value.length - 1]?.id ?? '')
}
...
<template>
  <div class="page">
    ...
    <div v-if="!connected" class="card">
      ...
      <div class="row">
        <input ref="codeInput" v-model="address" class="code" placeholder="ABCD 1234"
               autocapitalize="characters" autocomplete="off" spellcheck="false"
               @keyup.enter="connect()" />
        <button class="btn" :disabled="busy" @click="connect()">Подключиться</button>
      </div>
      <div class="row">
        <button class="ghost" @click="pasteCode">📋 Вставить</button>
        <button class="ghost" :disabled="!address" @click="clearCode">✕ Очистить</button>
      </div>
    ...
    <template v-else>
      ...
      <div v-if="screens.length > 1" class="card">
        <b>🖥 Экранов на этой приставке: {{ screens.length }}</b>
        <div class="pad">
          <button v-for="sc in screens" :key="sc.id" :class="{ wide: true, on: sc.id === screen }"
                  @click="pickScreen(sc.id)">
            {{ sc.name || 'Экран' }}
          </button>
        </div>
        <p class="hint">
          Плеер открыт в нескольких местах (например, на телевизоре и на компьютере).
          Команды, картинка с телефона и то, что показано ниже, — только для выбранного.
        </p>
      </div>

      <div v-if="cast" class="card">
        <b>📱 На экране показ с телефона</b>
        <div class="pad">
          <template v-if="cast.video">
            <button @click="cmd('back')">⏪ 30</button>
            <button class="wide" @click="cmd('playpause')">{{ cast.paused ? '▶' : '⏸' }}</button>
            <button @click="cmd('fwd')">⏩ 30</button>
          </template>
          <button @click="cmd('stop')">✕ Убрать с экрана</button>
        </div>
        <div v-if="cast.video" class="meta">{{ fmt(cast.time) }} / {{ fmt(cast.duration) }}</div>
        <p class="hint">
          Пока показ открыт, кнопки ниже тоже относятся к нему. Закроете — приставка
          вернётся к тому, что смотрели.
        </p>
      </div>
      ...
      <div class="card">
        <b>📤 С телефона на экран</b>
        <div class="pad">
          <button class="wide" :disabled="castProgress !== null" @click="pickCast">
            {{ castProgress !== null ? `Отправляю… ${castProgress}%` : '🖼 Фото или видео' }}
          </button>
          <button @click="stopCast">✕ Убрать</button>
        </div>
        <input ref="castInput" type="file" accept="image/*,video/*" hidden @change="sendCast" />
        <p class="hint">
          Из галереи сразу на ТВ, без медиатеки. Файл пару часов лежит на сервере по
          случайной ссылке и потом стирается. На приставке «Назад» закрывает показ.
        </p>
      </div>
...
.ghost { flex: 1; background: var(--bg-secondary); color: var(--text-color); border: 1px solid var(--border-color);
...
/* выбранный экран — как включённый режим */
.pad button.on { border-color: var(--accent, #4c8dff); color: var(--accent, #4c8dff); }
.scrub { width: 100%; }
