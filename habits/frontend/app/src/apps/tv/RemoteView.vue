...
function sendCast(e: Event) {
  ...
  xhr.open('POST', `${apiBase()}/tv/cast?room=${encodeURIComponent(lastKey)}&name=${encodeURIComponent(file.name)}`)
...
async function stopCast() {
  if (!lastKey) return
  try {
    await api.post(`/tv/cast/stop?room=${encodeURIComponent(lastKey)}`)
  } catch (e) {
