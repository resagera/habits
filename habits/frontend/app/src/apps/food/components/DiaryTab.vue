...
// «По плану на этот день» — подсказка из планов питания, у которых дата
// попадает в период. Грузится отдельно: дневник не должен ждать её и не
// должен падать, если планов нет.
const planToday = ref<FoodPlanToday[]>([])
const applying = ref(false)

async function loadPlan() {
  try {
    planToday.value = (await foodApi.fetchPlansForDate(day.value)).items.filter(
      (p) => p.slots.length > 0,
    )
  } catch {
    planToday.value = []
...
/** Перенос дня плана в дневник прямо из Дневника (mode add — «ещё раз»). */
async function applyPlanDay(p: FoodPlanToday, mode: '' | 'add') {
  applying.value = true
  try {
    const { result } = await foodApi.applyPlan(p.plan_id, {
    ...
    showToast(`Добавлено записей: ${result.created}`)
    await load()
  } catch (e) {
    showToast(e instanceof Error ? e.message : 'Не удалось перенести')
  } finally {
...
<template>
  ...
  <template v-else-if="diary">
    ...
    <div v-for="p in planToday" :key="p.plan_id" class="plan-hint card-glass">
      <div class="ph-head">
        <b>📅 По плану: {{ p.plan_name }}</b>
        <span class="ph-day">день {{ p.day_index + 1 }}</span>
      </div>
      <div class="ph-slots">
        <span v-for="(s, i) in p.slots" :key="s.id">
          {{ s.title || MEAL_TYPE_LABELS[s.meal_type] }}<template v-if="i < p.slots.length - 1">, </template>
        </span>
      </div>
      <button v-if="!p.applied" class="ph-btn" :disabled="applying" @click="applyPlanDay(p, '')">
        📋 Перенести в дневник
      </button>
      <div v-else class="ph-done">
        ✅ Уже перенесено ({{ p.applied }})
        <button class="ph-again" :disabled="applying" @click="applyPlanDay(p, 'add')">ещё раз</button>
      </div>
    </div>
      ...
      <div v-for="m in g.meals" :key="m.id" class="meal" @click="openEdit(m)">
        ...
        <div class="m-body">
          <div class="m-title">
            ...
            <span v-if="m.source_type === 'plan'" title="Из плана питания">📅</span>
          </div>
