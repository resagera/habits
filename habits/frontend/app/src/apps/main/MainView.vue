...
import { isMenuPinned, sortPages } from '../../shared/pageOrder'
...
// Админские страницы («Админка», «Релизы») на плитках раньше не появлялись,
// хотя в меню были: набор на главной и в меню должен совпадать.
// Порядок тот же, что в меню (shared/pageOrder): закреплённые, потом
// заполненные, потом пустые — иначе плитки и меню жили бы каждый своей жизнью.
const apps = computed(() =>
  sortPages([
    ...allApps.filter((r) => pageAllowed(String(r.name))),
    ...(isAdmin.value ? router.getRoutes().filter((r) => r.meta.admin) : []),
  ]),
)

// Блок плиток сворачивается: у кого страниц много, поиск и ближайшие
// напоминания оказываются глубоко внизу. Состояние — на сервере, как у
// остальных свёрнутых блоков: localStorage в вебвью Telegram чистится.
const TILES = 1
const tilesCollapsed = ref(false)

function toggleTiles() {
  tilesCollapsed.value = !tilesCollapsed.value
  saveCollapsed('main', tilesCollapsed.value ? new Set([TILES]) : new Set())
}
...
onMounted(async () => {
  tilesCollapsed.value = (await loadCollapsed('main')).has(TILES)
...
<template>
  ...
  <button class="intro-row" @click="toggleTiles">
    <span class="intro">Все мини-приложения в одном месте. Выберите, с чего начать:</span>
    <span class="chev">{{ tilesCollapsed ? '▸' : '▾' }}</span>
  </button>

  <div v-show="!tilesCollapsed" class="tiles">
    <RouterLink v-for="app in apps" :key="app.path" :to="app.path" class="tile">
      <span v-if="isMenuPinned(String(app.name))" class="tile-pin" title="Закреплено">📌</span>
