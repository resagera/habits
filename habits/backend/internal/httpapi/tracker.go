...
// Цвет отметки: обычный #rrggbb или пара '#rrggbb,#rrggbb' — градиент из
// набора. Отметка хранит цвет целиком, поэтому и рисуется без оглядки на
// набор: удалили цвет из набора — история осталась прежней.
var markColorRe = regexp.MustCompile(`^#[0-9a-fA-F]{6}(,#[0-9a-fA-F]{6})?$`)

const maxPaletteColors = 12

// validPalette проверяет набор именованных цветов и нормализует названия.
// Возвращает текст ошибки или пустую строку.
func validPalette(palette []store.CategoryColor) string {
	if len(palette) > maxPaletteColors {
		return "too many colors in palette"
	}
	for i := range palette {
		palette[i].Name = strings.TrimSpace(palette[i].Name)
		if !colorRe.MatchString(palette[i].Color) {
			return "palette color must be #rrggbb"
		}
		if palette[i].Color2 != "" && !colorRe.MatchString(palette[i].Color2) {
			return "palette gradient color must be #rrggbb"
		}
		if n := utf8.RuneCountInString(palette[i].Name); n < 1 || n > 40 {
			return "palette color name must be 1-40 characters"
		}
	}
	return ""
}
...
func (h *trackerHandlers) updateCategory(w http.ResponseWriter, r *http.Request) {
	...
	var req struct {
		...
		Multi    *bool                  `json:"multi"`
		Emoji    *string                `json:"emoji"`
		Palette  *[]store.CategoryColor `json:"palette"`
	}
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		badRequest(w, "invalid JSON body")
		return
	...
	if req.Palette != nil {
		if msg := validPalette(*req.Palette); msg != "" {
			badRequest(w, msg)
			return
		}
	}
...
func (h *trackerHandlers) toggleMark(w http.ResponseWriter, r *http.Request) {
	...
	if req.Color != nil && !markColorRe.MatchString(*req.Color) {
		badRequest(w, "color must be #rrggbb or a #rrggbb,#rrggbb gradient pair")
		return
	}
	if req.Emoji != nil && !validEmoji(*req.Emoji) {
		badRequest(w, "invalid emoji")
