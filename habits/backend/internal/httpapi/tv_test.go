...
// Пульт должен отличать экраны даже от старой страницы, которая номер не
// шлёт: отправителя проставляет шина.
func TestTagSender(t *testing.T) {
	cases := []struct{ in, want string }{
		{`{"t":"state","video":null}`, `{"s":"s3","t":"state","video":null}`},
		{`{}`, `{"s":"s3"}`},
		{"\n  {\"t\":\"jobs\"}", `{"s":"s3","t":"jobs"}`},
		{`[1,2]`, `[1,2]`}, // не объект — не трогаем
		{``, ``},
	}
	for _, c := range cases {
		if got := string(tagSender([]byte(c.in), "s3")); got != c.want {
			t.Errorf("tagSender(%q) = %q, ожидалось %q", c.in, got, c.want)
		}
	}
	// страница новее шины — её собственное поле должно победить (в JSON
	// выигрывает последнее вхождение ключа)
	var out map[string]any
	if err := json.Unmarshal(tagSender([]byte(`{"s":"s9","t":"state"}`), "s3"), &out); err != nil || out["s"] != "s9" {
		t.Errorf("своё поле страницы: %v %v", out, err)
	}
}
