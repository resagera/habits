...
func (h *tvHandlers) castUpload(w http.ResponseWriter, r *http.Request) {
	...
	// пульт говорит, на какой экран: в комнате их может быть несколько
	if screen := r.URL.Query().Get("screen"); screen != "" {
		if !h.hub.SendTo(key, screen, msg) {
			_ = os.Remove(h.cast.path(token))
			_ = os.Remove(h.cast.path(token) + ".type")
			writeError(w, http.StatusConflict, "offline", "этот экран уже не на связи — выберите другой")
			return
		}
	} else {
