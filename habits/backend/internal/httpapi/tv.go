...
// tagSender добавляет в JSON-объект поле «s» — номер приславшего экрана.
// Склейка байтов, а не разбор и сборка: в сообщении бывает листинг папки на
// сотни файлов, и перекладывать его целиком ради одного поля незачем.
func tagSender(data []byte, id string) []byte {
	trimmed := bytes.TrimLeft(data, " \t\r\n")
	if len(trimmed) < 2 || trimmed[0] != '{' {
		return data
	}
	head := `{"s":"` + id + `"`
	if trimmed[1] == '}' { // пустой объект — запятая не нужна
		return append([]byte(head), trimmed[1:]...)
	}
	return append([]byte(head+","), trimmed[1:]...)
}
...
func (h *tvHandlers) serve(w http.ResponseWriter, r *http.Request, key string, role tvrelay.Role) {
	...
	if role == tvrelay.RoleTV {
		// свой номер: в комнате (у одного агента) плееров бывает несколько —
		// на телевизоре и на компьютере, и пульт должен их различать
		msg, _ := json.Marshal(map[string]any{"t": "screen", "id": c.ID()})
		c.Send(msg)
	...
	for {
		...
		// Шина по-прежнему не знает, что внутри сообщений: читает только «кому»
		// и как назвался экран. Остальное — непрозрачный JSON.
		var head struct {
			T    string `json:"t"`
			To   string `json:"to"`
		...
		if role == tvrelay.RoleTV && head.T == "iam" {
			c.SetName(head.Name)
			h.announce(key)
			continue
		}
		if role == tvrelay.RoleRemote && head.To != "" {
			h.hub.SendTo(key, head.To, data)
			continue
		}
		if role == tvrelay.RoleTV {
			// Проставляем отправителя сами, не разбирая остальное: пульт
			// должен отличать экраны и от старых страниц, которые про номера
			// ещё не знают (их не перезагрузили после обновления агента).
			// Своё поле идёт первым — если страница пришлёт своё, победит оно.
			data = tagSender(data, c.ID())
		}
...
func (h *tvHandlers) announce(key string) {
	...
	msg, _ := json.Marshal(map[string]any{
		"t": "presence", "tv": tv, "remotes": remotes, "screens": h.hub.Screens(key),
	})
