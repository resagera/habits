...
// POST /settings/tokens/revoke-all — «выйти на всех устройствах»: отзывает
// все токены сразу. Сеанс в Telegram при этом остаётся: его подпись выдаёт
// Telegram, приложение отозвать её не может — так и написано в интерфейсе.
func (h *tokensHandlers) revokeAll(w http.ResponseWriter, r *http.Request) {
	user := auth.UserFromContext(r.Context())
	n, err := h.store.RevokeAllAccessTokens(r.Context(), user.ID)
	if err != nil {
		internalError(w)
		return
	}
