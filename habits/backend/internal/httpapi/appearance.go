...
func (h *appearanceHandlers) set(w http.ResponseWriter, r *http.Request) {
	...
	// 0 — «как есть»; за пределами 70–150 % интерфейсом уже не пользоваться
	if req.Scale != 0 && (req.Scale < 70 || req.Scale > 150) {
		badRequest(w, "scale must be 70-150 or 0")
		return
	}
...
// galleryImageResp — картинка галереи вместе с весом файла.
type galleryImageResp struct {
	store.GalleryImage
...
func (h *appearanceHandlers) gallery(w http.ResponseWriter, r *http.Request) {
	...
	// вес файла — для просмотрщика на весь экран (в плитке он не нужен)
	dir := filepath.Join(h.dataDir, galleryDir)
	out := make([]galleryImageResp, 0, len(images))
	for _, img := range images {
		var size int64
		if st, err := os.Stat(filepath.Join(dir, img.Filename)); err == nil {
			size = st.Size()
		}
		out = append(out, galleryImageResp{GalleryImage: img, Size: size})
	}
	writeJSON(w, http.StatusOK, map[string]any{"categories": cats, "images": out})
}
