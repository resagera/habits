...
// SelfProfile — карточка «кто я» для страницы настроек: те же поля, что видит
// админ в чужой карточке, но без счётчиков данных и без бан-статуса как
// действия. Отдельный метод, чтобы не звать админский GetAdminUser: он
// считает ещё восемь COUNT(*), которые здесь не нужны.
func (s *Store) SelfProfile(ctx context.Context, id int64) (AdminUser, []UserDevice, error) {
	var u AdminUser
	err := s.pool.QueryRow(ctx, `
		SELECT id, COALESCE(username, ''), COALESCE(first_name, ''),
		       created_at, last_seen_at, banned, user_type
	...
	rows, err := s.pool.Query(ctx, `
		SELECT ip, device, created_at FROM user_devices
		WHERE user_id = $1 ORDER BY created_at DESC LIMIT 100`, id)
	if err != nil {
		return u, nil, err
	}
	devices, err := pgx.CollectRows(rows, pgx.RowToStructByPos[UserDevice])
