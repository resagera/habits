...
func (s *Store) resolveCheckerGroup(ctx context.Context, ownerID, groupID int64) (resolvedCheckGroup, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id, name FROM checker_groups WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL`,
...
func (s *Store) fillCheckerSubtree(ctx context.Context, g *resolvedCheckGroup, level int) error {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT id, name FROM checker_groups WHERE parent_id = $1 AND deleted_at IS NULL ORDER BY position, id`, g.ID)
	if err != nil {
		return err
	}
