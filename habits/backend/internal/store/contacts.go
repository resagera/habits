...
func (s *Store) ShareTitle(ctx context.Context, kind string, ownerID, refID int64) (string, error) {
	...
	case "checker_group":
		q = `SELECT name FROM checker_groups WHERE id = $1 AND user_id = $2 AND parent_id IS NULL AND deleted_at IS NULL`
	case "checker_share":
		q = `SELECT name FROM checker_groups WHERE id = $1 AND user_id = $2 AND parent_id IS NULL AND deleted_at IS NULL`
	...
	case "food_plan":
		q = `SELECT name FROM food_plans WHERE id = $1 AND user_id = $2`
	case "vault_folder", "vault_file":
		// у сейфа название достаётся отдельно: имя файла зашифровано
		return s.vaultShareTitle(ctx, kind, ownerID, refID)
	default:
...
func (s *Store) ApplyShare(ctx context.Context, kind string, fromUser, refID, toUser int64) (string, error) {
	...
	case "checker_share":
		return s.ShareCheckerGroupAccess(ctx, fromUser, refID, toUser)
	case "reminder_category":
	...
	case "food_plan":
		return s.ShareFoodPlan(ctx, fromUser, refID, toUser)
	case "vault_folder", "vault_file":
		return s.ShareVault(ctx, kind, fromUser, refID, toUser)
	}
	return "", errors.New("unknown share kind")
}
