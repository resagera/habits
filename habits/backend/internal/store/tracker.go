...
type Category struct {
	...
	Palette []CategoryColor `json:"palette"`
	OwnerID int64           `json:"owner_id"`
	Mine    bool            `json:"mine"`
	Shared  bool            `json:"shared"` // есть участники (у владельца) или получен по шарингу
}

// CategoryColor — цвет набора с названием. Непустой Color2 — градиент.
// Название нужно только человеку в настройках: на отметках оно не рисуется.
...
const categoryCols = `c.id, c.name, c.color, c.position, c.daily, c.kind, c.style, c.multi, c.emoji,
	c.palette, c.user_id, (c.user_id = $1),
...
func (s *Store) CreateCategory(ctx context.Context, userID int64, name, color, kind, style, emoji string, multi bool) (Category, error) {
	...
	err := s.pool.QueryRow(ctx, `
		...
		&c.Palette, &c.OwnerID, &c.Mine, &c.Shared)
...
func (s *Store) UpdateCategory(ctx context.Context, userID, id int64, p CategoryPatch) (Category, error) {
	var c Category
	// Набор цветов приходит целиком или не приходит вовсе: NULL в $11 значит
	// «не трогать», поэтому COALESCE, а не отдельный запрос.
	var palette []byte
	if p.Palette != nil {
		raw, err := json.Marshal(*p.Palette)
		if err != nil {
			return c, err
		}
		palette = raw
	...
	err := s.pool.QueryRow(ctx, `
		WITH upd AS (
			...
			SET name = COALESCE($3, name),
			    ...
			    palette = COALESCE($11::jsonb, palette),
			    updated_at = now()
			WHERE id = $2 AND user_id = $1
			RETURNING *
		)
		SELECT `+categoryCols+` FROM upd c`,
		userID, id, p.Name, p.Color, p.Position, p.Daily, p.Kind, p.Style, p.Multi, p.Emoji, palette).Scan(
		&c.ID, &c.Name, &c.Color, &c.Position, &c.Daily, &c.Kind, &c.Style, &c.Multi, &c.Emoji,
		&c.Palette, &c.OwnerID, &c.Mine, &c.Shared)
