...
// RevokeAllAccessTokens отзывает все активные токены пользователя («выйти
// везде»). Сеансы в Telegram этим не трогаются: initData подписывает Telegram,
// и отозвать его приложение не может.
func (s *Store) RevokeAllAccessTokens(ctx context.Context, userID int64) (int64, error) {
	tag, err := s.pool.Exec(ctx, `
		UPDATE access_tokens SET revoked_at = now()
