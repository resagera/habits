...
type client struct {
	...
	// id и название экрана: в одной комнате (у одного агента) плееров может
	// быть несколько — на телевизоре и на компьютере. Пульт выбирает, каким
	// управлять, иначе команды уходят обоим, а состояние шлют оба.
	id   string
	name string
}
...
// Screen — плеер в комнате: его id и как он себя назвал.
type Screen struct {
	ID   string `json:"id"`
...
type Hub struct {
	...
	seq   int64             // счётчик номеров экранов
}
...
func (h *Hub) Join(key string, role Role, ws *websocket.Conn) (*client, func()) {
	...
	h.mu.Lock()
	h.seq++
	c.id = "s" + strconv.FormatInt(h.seq, 36)
	r := h.rooms[key]
	if r == nil {
...
// SendTo — сообщение одному плееру комнаты: пульт управляет выбранным
// экраном, а не всеми сразу. false — такого экрана в комнате уже нет.
func (h *Hub) SendTo(key, id string, data []byte) bool {
	h.mu.RLock()
	r := h.rooms[key]
	var target *client
	if r != nil {
		for c := range r.clients {
			if c.role == RoleTV && c.id == id {
				target = c
			}
		}
	}
...
// Screens — плееры комнаты по порядку появления: пульту нужен их список,
// чтобы дать выбрать экран.
func (h *Hub) Screens(key string) []Screen {
	h.mu.RLock()
	defer h.mu.RUnlock()
	out := []Screen{}
	r := h.rooms[key]
	...
	for c := range r.clients {
		...
		name := c.name
		c.mu.Unlock()
		out = append(out, Screen{ID: c.id, Name: name})
	}
