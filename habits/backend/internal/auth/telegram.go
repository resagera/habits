...
type TgUser struct {
	...
	// TokenSession — вход по токену доступа (веб/расширение), а не по
	// initData из Telegram. Такая сессия не получает админ-полномочий и не
	// может выпускать новые токены (см. auth.Middleware.Wrap).
	TokenSession bool `json:"-"`
	// TokenID — id использованного токена (0 для initData-сессии).
	TokenID int64 `json:"-"`
}
