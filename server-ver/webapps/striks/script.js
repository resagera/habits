...
async function apiLoadLinks() {
    try {
        const res = await fetch(`${apiBaseV3}/links?userId=${userId}`);
        if (!res.ok) throw new Error(await res.text());
        const data = await res.json();
        linksTree = Array.isArray(data) ? data : [];
        saveLinksTree();
        renderLinksTree();
        console.log("✅ Загружено из API:", data.length);
    } catch (err) {
        console.error("Ошибка загрузки из API:", err);
    }
}

// Рекурсивно "расплющивает" дерево ссылок в плоский массив
function flattenLinks(tree, parentPath = "root") {
    let flat = [];
    for (const node of tree) {
        if (node.type === "link") {
            ...
            const newPath = parentPath ? `${parentPath}.${sanitizePath(node.name)}` : sanitizePath(node.name);
            flat = flat.concat(flattenLinks(node.children, newPath));
        }
    }
    return flat;
}

// Очистка имени для path (без пробелов и спецсимволов)
function sanitizePath(name) {
    return name.replace(/[^\w\d_-]+/g, "_").toLowerCase();
}

// Выгрузка всех локальных ссылок в API
async function uploadLocalLinksToAPI() {
    ...
    try {
        const flatLinks = flattenLinks(localLinks);
        console.log("🔄 Готовим к отправке:", flatLinks.length, "записей");
...
document.getElementById("upload-links-to-api-btn").onclick = uploadLocalLinksToAPI;
...
document.getElementById("save-new-folder").onclick = () => {
